...
frontend/
  ...
  vendor/                    # Self-hosted dependencies (NO runtime CDNs)
    ...
    tailwind.css             # Built by Tailwind CLI (run `npm run build:css`)
...
**No runtime CDNs.** All JS/CSS dependencies are self-hosted in `frontend/vendor/` and served
directly from Firebase Hosting. External CDNs are blocked on some networks; if Alpine.js fails to
load the entire reactive system is dead.

### Tailwind CSS — Rebuild Before Every Frontend Deploy

Tailwind CSS is generated by the CLI from `tailwind.config.js`. The output `frontend/vendor/tailwind.css`
must be rebuilt whenever HTML or JS files change (new utility classes may have been added).
...
> The custom theme (colors, fonts, border-radius) lives in `tailwind.config.js` — NOT in `index.html`.
> After changing the theme, rebuild and commit `frontend/vendor/tailwind.css`.

### Adding a New Vendored Dependency

1. `curl -sL <url> -o frontend/vendor/<file>` — download the file locally
2. Update the `<script src="vendor/...">` tag in `index.html`
3. Commit the vendor file
