...
- [x] **Implement Unit Test Coverage (Zero tests currently)**
  ...
  - **Fix Details**: Established the `pytest` framework and added comprehensive unit tests for pure functionalities in `algos/`. Mocked the `DatabaseProvider` and `StorageProvider` for the service features (`ProjectService` & `PhotoService`) to isolate tests from actual database/storage operations.
- [x] **Refactor `main.py` Monolith**
  ...
  - **Fix Details**: Reorganized the backend codebase into a vertical slice architecture under `backend/features/`. Separated logic into `routers.py`, `service.py`, and `models.py` for each feature (`auth`, `gphotos`, `photo`, `project`, `export`). Switched to using FastAPI's `Depends()` for dependency injection, cleanly decoupling infrastructure providers from the core logic.

## [SEV-2] Code Idioms & Reliability Patterns
- [x] **Resolve Broad Exception Swallowing**
